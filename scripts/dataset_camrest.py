...
class BaseDataset(Dataset):
    ...
    def match_kgentry(self, kg, gold):
       ...
       #  return self.kg2seq(kgitem.strip())
            ...
            if dialog["kg"]:
                if self.split_type == "train":    # check for the gold knowledge entry using the information from reference entities
                    ...
                    if self.args.ir == "dense":
                        ...
                        #print("dense_used_knowledge1-",used_knowledge)
                        #print("dense_used_knowledge1_type-",type(used_knowledge))
                        #used_knowledge = [self.tokenizer.tokenize(entry)for entry in used_knowledge]
                        used_knowledge = self.tokenizer.tokenize(used_knowledge)
                        #print("dense_used_knowledge2-",used_knowledge)
                        ...
                        for key, vals in Dict.items():
                            ...
                            for val in vals:
                                ...
                                for k, v in knowledge.items():
                                    kgitem += " "+k+" "+v
                        #used_knowledge = [self.kg2seq(kgitem.strip())]
                        #used_knowledge = used_knowledge[0].split()
                        print("kgitem",kgitem)
                        print("kgitem_type",type(kgitem))
                        used_knowledge = self.tokenizer.tokenize(kgitem)
                if self.args.ir == "dense":
                    pass
                  
                   #used_knowledge = [self.kg2seq(entry)for entry in used_knowledge]
