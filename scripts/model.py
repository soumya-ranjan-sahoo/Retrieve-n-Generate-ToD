...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        #print(position_ids.size(), position_embeds.size())

        #print(inputs_embeds.size(), position_embeds.size(), dep_embs.size(),deplvl_embs.size(), postag_embs.size())
        #summed_embedding = dep_embs + deplvl_embs + postag_embs
        summed_embedding = inputs_embeds  + position_embeds 
        #hidden_states = self.ln_f(hidden_states) # layer normalization
        #hidden_states = self.drop(hidden_states)

        enc_inp = summed_embedding
        non_pad_mask = get_non_pad_mask(input_ids, self.pad_idx)
        slf_attn_mask = get_attn_key_pad_mask(seq_k=input_ids, seq_q=input_ids, pad_idx=self.pad_idx)
        ...
        for layer in self.encoder:
            ''' Passes through 2 layers on encoders ; more layers can be added in the forward method in class EncoderLayer'''
            enc_inp, _ = layer(enc_inp, non_pad_mask, slf_attn_mask)
        enc_output = enc_inp


        #hidden_states = enc_output#self.wte(input_ids)+self.wpe(position_ids)
...
def run_batch_generation(args, model, batch):
    ...
    input_ids, pos_ids, lm_labels = batch
    ...
    model_outputs = model(input_ids=input_ids, position_ids=None, labels=lm_labels)
...
def run_batch_generation_sample(args, model, batch, dataset):
    ...
    return current_output, response_text, dialog_id, ref_entities, knowledge_text, task
