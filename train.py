...
import torch
#import wandb
#wandb.init(project="train-logs-project")
...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, run_batch_fn_train, run_batch_fn_eval) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            #print("batch size in train.py",len(batch))
            model.train()
...
def main():
    ...
    # Required parameters
    parser.add_argument("--params_file", type=str, default="config/gpt2/params.json", help="JSON configuration file")
    ...
    parser.add_argument("--dataset", type=str, default="camrest", choices=["incar","camrest", "woz2_1"],
                        help="dataset name.")
    parser.add_argument("--ir", type=str, default="sparse", choices=["dense","sparse"], help="retrieval method")
    parser.add_argument("--topk", type=int, default=2, help="topk retrieved KB records using IR")
    ...
    parser.add_argument("--exp_name", type=str, default="gpt2",
    ...
    if args.dataset=="incar":
        ...
        from scripts.dataset_multiwoz import EvalDataset, Dataset, SPECIAL_TOKENS
    ...
    if args.local_rank in [-1, 0]:
        ...
        print("------------Instantiated Eval Dataset ---------------------------------------")
